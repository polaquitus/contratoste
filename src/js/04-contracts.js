...
function renderDossierHtml(c){
  ...
  // Invitados = todos los cargados en "Oferentes Invitados"; Ofertas Recibidas = el
  // subconjunto que efectivamente cotiz\u00f3 (cotizo!==false) \u2014 mismo dato, dos vistas, ambas
  // con nombre y cantidad (antes solo se ve\u00eda un n\u00famero, mezclado entre las dos secciones).
  function ofrList(list){
    return list.length?list.map(function(o,i){return '<div class="ofr-row"><span class="ofr-num">'+(i+1)+'</span><span class="ofr-name">'+_e(o.nombre||o.name||String(o))+'</span></div>';}).join(''):'<div class="empty-cell" style="padding:8px 0">Sin registros</div>';
  }
  var recibidos=ofrs.filter(function(o){return o.cotizo!==false;});
  var invitadosHtml=ofrs.length?ofrList(ofrs):(c.oferentes?'<div style="font-size:12.5px;color:#374151;padding:4px 0">'+_e(c.oferentes)+'</div>':'<div class="empty-cell" style="padding:8px 0">Sin oferentes registrados</div>');
  var recibidosHtml=ofrList(recibidos);
...
+(c.com?'':'')
+'<div class="ofrs">'
+'<div><div class="oftit">Invitados <span class="ofcnt">'+ofrs.length+'</span></div>'+invitadosHtml+'</div>'
+'<div><div class="oftit">Ofertas recibidas <span class="ofcnt">'+recibidos.length+'</span></div>'+recibidosHtml+'</div>'
+'</div>'
+'<div class="ofrs" style="margin-top:14px">'
