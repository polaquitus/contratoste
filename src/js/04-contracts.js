...
function renderDossierHtml(c){
  ...
  // Fuente primaria: los campos "Participantes" (invitados) y "Oferentes" (los que
  // efectivamente cotizaron) del propio contrato, cargados en Datos RFQ \u2014 son los que
  // realmente se completan en el d\u00eda a d\u00eda. LICIT_DB (m\u00f3dulo Licitaciones) queda como
  // fallback solo si el contrato no tiene esos campos cargados.
  ...
  var invitadosArr=particArr.length?particArr:licitOfrs.map(function(o){return o.nombre||o.name||String(o);});
  var recibidosArr=ofrArr.length?ofrArr:licitOfrs.filter(function(o){return o.cotizo!==false;}).map(function(o){return o.nombre||o.name||String(o);});
  var nPartic=invitadosArr.length||'\u2014';
  var nOfrs=(c.cof!=null&&c.cof!=='')?c.cof:(recibidosArr.length||'\u2014');
...
function editCont(id){const c=window.DB.find(x=>x.id===id);if(!c)return;document.getElementById('formErrBanner')?.remove();editId=id;
  ...
  document.getElementById('f_partic').value=c.participantes||'';
