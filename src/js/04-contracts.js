...
function renderDossierHtml(c){
  ...
  var daysToDD='\u2014';
  if(c.fev){var _dd=new Date(c.fev+'T00:00:00'),_hoy=new Date();_hoy.setHours(0,0,0,0);if(!isNaN(_dd)){daysToDD=Math.ceil((_dd-_hoy)/86400000);}}
  function kv(lbl,val,full){return '<div class="kvi"'+(full?' style="grid-column:1/-1"':'')+'><div class="kvl">'+lbl+'</div><div class="kvv">'+val+'</div></div>';}
  var enmRows=enms.length?enms.map(function(e,i){var ts=(typeof enmTipos==='function'?enmTipos(e):[e.tipo||'otro'])||['otro'];var tagsHtml=ts.map(function(t){var tc2=t==='ACTUALIZACION_TARIFAS'?'tar':t==='EXTENSION'?'ext':t==='SCOPE'?'scope':t==='CLAUSULAS'?'claus':'otro';return '<span class="tag '+tc2+'">'+_e(t)+'</span>';}).join(' ');return '<tr><td>'+(i+1)+'</td><td>'+_e(e.num)+'</td><td>'+_d(e.fecha)+'</td><td>'+tagsHtml+'</td><td>'+_e(e.descripcion||'\u2014')+'</td></tr>';}).join(''):'<tr><td colspan="5" class="empty-cell">Sin enmiendas registradas</td></tr>';
  var aveRows=aves.length?aves.slice().sort(function(a,b){return new Date(a.fecha)-new Date(b.fecha);}).map(function(a){return '<tr><td><span class="tag '+(a.tipo==='POLINOMICA'?'poly':'owner')+'">'+(a.tipo==='POLINOMICA'?'&#x1F504; Polin\u00f3mica':'&#x1F538; Otros (Owner)')+'</span>'+(a.autoGenerated?'<span class="tag auto" style="margin-left:4px">AUTO</span>':'')+'</td><td>'+_d(a.fecha)+'</td><td>'+_e(a.periodo||'\u2014')+'</td><td class="mono">+ '+_m(a.monto||0)+' '+_e(c.mon||'ARS')+'</td><td>'+_e(a.concepto||'\u2014')+'</td></tr>';}).join(''):'<tr><td colspan="5" class="empty-cell">Sin AVEs registrados</td></tr>';
  var ofrsHtml=ofrs.length?ofrs.map(function(o,i){var cotizo=o.cotizo!==false;return '<div class="ofr-row"><span class="ofr-num">'+(i+1)+'</span><span class="ofr-name">'+_e(o.nombre||o.name||String(o))+'</span>'+(cotizo?'<span class="otag si">Cotiz\u00f3</span>':'<span class="otag no">No cotiz\u00f3</span>')+'</div>';}).join(''):(c.oferentes?'<div style="font-size:12.5px;color:#374151;padding:4px 0">'+_e(c.oferentes)+'</div>':'<div class="empty-cell" style="padding:8px 0">Sin oferentes registrados</div>');
...
+'.grid2b{display:grid;grid-template-columns:1fr 1fr 1fr;gap:18px;align-items:start}'
...
+'.chk-row{display:flex;align-items:center;gap:9px;padding:8px 16px;border-bottom:1px solid #f1f3f6;font-size:13px}'
...
+'.sgl{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;padding:14px 16px}'
...
+'.tag.poly{background:var(--poly-bg);color:var(--poly)}.tag.owner{background:var(--owner-bg);color:var(--owner)}.tag.auto{background:#e0f2fe;color:#0369a1;margin-left:4px}'
...
+kv('Metier',_e(c.tcontr||'\u2014'))
+kv('UTE',_e(c.asset||c.own||'APE + SR'))
+kv('Contract #','<strong>'+_e(c.num||'\u2014')+'</strong>')
+kv('Vigencia',_d(c.fechaIni)+' \u2192 '+_d(c.fechaFin),true)
...
+kv('Participantes',String(nPartic))
+kv('Ofertas recibidas',String(nOfrs))
+kv('Apertura simult\u00e1nea','YES')
+kv('Mejor precio','YES')
+'</div>'+(c.com?'<div style="padding:4px 4px 10px"><div class="kvl">Justificaci\u00f3n</div><div style="font-size:12.5px;color:#6b7280;font-style:italic;margin-top:3px">'+_e(c.com)+'</div></div>':'')+'</div></div>'
+'</div>'
+'<div>'
+'<div class="card"><div class="card-title">Evaluaci\u00f3n</div>'
+'<div class="chk-row">'+chk(c.dd)+'<span>DD / Pre-risk</span></div>'
+'<div class="chk-row">'+chk(c.pr)+'<span>E-valuarte</span></div>'
+'<div class="chk-row">'+chk(c.sq)+'<span>Sequana</span></div>'
+'<div class="chk-row">'+chk(false)+'<span>Sustainability</span></div>'
+'</div>'
...
+'</div></div>'
+'<div class="card" style="grid-column:span 1"><div class="card-title">Firmas</div><div class="sgl" style="grid-template-columns:1fr">'
...
+(c.com?'':'')
+'<div class="ofrs">'
+'<div><div class="oftit">Oferentes invitados</div>'+ofrsHtml+'</div>'
+'<div><div class="oftit">Aprobados t\u00e9cnicamente</div>'
+'<div style="font-size:13px;color:#374151;padding:4px 0">'+(c.oferentes?_e(c.oferentes):'\u2014')+'</div></div>'
+'</div>'
+'</div>'
+'</div>'
+'<div class="page">'
...
+'<div class="fin-tile owner"><div class="fin-label">Total Otras AVE<span class="fin-tag">Owner / CC</span></div><div class="fin-value">'+_m(aveOwner)+' '+_e(c.mon||'ARS')+'</div><div class="fin-usd">'+aveOwnerList.length+' AVE(s)</div></div>'
  ...
  if(tipos.includes('ACTUALIZACION_TARIFAS')){
    ...
    if(aveManualVal!==null){
      ...
      if(aveManualVal>0){
        ...
        cc.aves.push({id:Date.now().toString(36)+Math.random().toString(36).substr(2,4), tipo:'POLINOMICA', subtipo:'MANUAL', concepto:'Polinómica manual — Enm.'+num+(allNotes?(' · '+allNotes):''), monto:Math.round(aveManualVal*100)/100, enmRef:num, periodo:lastR.newPer, autoGenerated:false, fecha:new Date().toISOString()});
      }
    } else {
      results.forEach((r,idx)=>{
        if(r.aveMonto>0){
          ...
          const aveObj={id:Date.now().toString(36)+Math.random().toString(36).substr(2,4)+'_'+idx, tipo:'POLINOMICA', subtipo:'AUTO', concepto:'Polinómica auto — Enm.'+num+tag+' (+'+((r.pct)*100).toFixed(2)+'%)'+(notes?(' · '+notes):''), monto:Math.round(r.aveMonto*100)/100, enmRef:num, periodo:r.newPer, autoGenerated:true, fecha:new Date().toISOString()};
    ...
    (cc.aves||[]).filter(a=>a.tipo==='OWNER'&&a.ajustable).forEach(a=>{
      ...
      cc.aves.push({
        ...
        monto:delta, enmRef:num, periodo:lastNewPer, autoGenerated:true, ownerAveRef:a.id,
