...
function renderDossierHtml(c){
  ...
  var aveColCount=isUsdContract?6:7;
  var aveRows=aves.length?aves.slice().sort(function(a,b){return new Date(a.fecha)-new Date(b.fecha);}).map(function(a){
    ...
    var tipoBdg='<span class="tag '+(isPoly?'poly':'owner')+'">'+(isPoly?'\u{1F504} AVE Polin\u00f3mica':'\u{1F535} AVE Owner')+'</span>';
    var subBdgs='';
    if(a.autoGenerated) subBdgs+=' <span class="tag auto">AUTO</span>';
    if(a.ajustable) subBdgs+=' <span class="tag ajust">\u{1F517} Ajustable</span>';
    if(a.ownerAveRef) subBdgs+=' <span class="tag ajust">\u{1F517} s/Owner</span>';
    var usdCell='';
    if(!isUsdContract){
      ...
      usdCell='<td class="mono" style="text-align:right">'+_m(usdVal)+' USD'+estTag+'</td>';
    }
    return '<tr><td>'+tipoBdg+subBdgs+'</td><td>'+_d((a.fecha||'').substring(0,10))+'</td><td>'+_e(a.periodo||'\u2014')+'</td>'
      +'<td class="mono" style="text-align:right">+'+_m(a.monto||0)+' '+_e(c.mon||'ARS')+'</td>'
      +usdCell
      +'<td>'+_e(a.concepto||'\u2014')+'</td>'
      +'<td class="mono" style="font-size:11px">'+(a.enmRef?'Enm. N\u00b0'+a.enmRef:'\u2014')+'</td></tr>';
...
+'</div>'
+'<table><thead><tr><th>Tipo</th><th>Fecha</th><th>Per\u00edodo</th><th>Monto</th>'+(isUsdContract?'':'<th>Eq. USD</th>')+'<th>Concepto</th><th>Enm. asociada</th></tr></thead><tbody>'+aveRows+'</tbody></table>'
+'</div>'
+'</div>'
+'</body></html>';
}
